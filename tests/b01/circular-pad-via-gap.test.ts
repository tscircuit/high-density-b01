...
test("B01 fits a via between circular pads without replacing them with boxes", () => {
  ...
  for (const resolution of [
    { highResolutionCellSize: 0.01, lowResolutionCellSize: 0.02 },
    { highResolutionCellSize: 0.1, lowResolutionCellSize: 0.4 },
  ]) {
    ...
    console.log(
      JSON.stringify({
        circularPadViaGap: {
          ...
          viaCenters: Array.from(solver.viaAllowed).flatMap(
            (allowed, cellId) =>
              allowed
                ? [
                    {
                      x: solver.cellCenterX[cellId],
                      y: solver.cellCenterY[cellId],
                    },
                  ]
                : [],
          ),
