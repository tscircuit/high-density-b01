...
function getSquaredDistanceFromPointToSegment(params: {
  point: Point2d
  segmentStart: Point2d
  segmentEnd: Point2d
}): number {
  const segmentDx = params.segmentEnd.x - params.segmentStart.x
  const segmentDy = params.segmentEnd.y - params.segmentStart.y
  const segmentLengthSquared = segmentDx ** 2 + segmentDy ** 2
  if (segmentLengthSquared === 0) {
    return (
      (params.point.x - params.segmentStart.x) ** 2 +
      (params.point.y - params.segmentStart.y) ** 2
    )
  }
  const projection = clamp(
    ((params.point.x - params.segmentStart.x) * segmentDx +
      (params.point.y - params.segmentStart.y) * segmentDy) /
      segmentLengthSquared,
    0,
    1,
  )
  const closestX = params.segmentStart.x + projection * segmentDx
  const closestY = params.segmentStart.y + projection * segmentDy
  return (params.point.x - closestX) ** 2 + (params.point.y - closestY) ** 2
}

function getSquaredDistanceFromPointToRect(params: {
  point: Point2d
  rect: RectBounds
}): number {
  const nearestX = clamp(params.point.x, params.rect.minX, params.rect.maxX)
  const nearestY = clamp(params.point.y, params.rect.minY, params.rect.maxY)
  const dx = params.point.x - nearestX
  const dy = params.point.y - nearestY
  return dx * dx + dy * dy
}
...
function getOrientation(params: {
  ...
  return (
    (params.second.x - params.first.x) * (params.third.y - params.first.y) -
    (params.second.y - params.first.y) * (params.third.x - params.first.x)
  )
}

function isPointOnSegment(params: {
  point: Point2d
  segmentStart: Point2d
  segmentEnd: Point2d
}): boolean {
  ...
  if (
    Math.abs(
      getOrientation({
        first: params.segmentStart,
        second: params.segmentEnd,
        third: params.point,
      }),
    ) > epsilon
  ) {
  ...
  return (
    params.point.x >= Math.min(params.segmentStart.x, params.segmentEnd.x) &&
    params.point.x <= Math.max(params.segmentStart.x, params.segmentEnd.x) &&
    params.point.y >= Math.min(params.segmentStart.y, params.segmentEnd.y) &&
    params.point.y <= Math.max(params.segmentStart.y, params.segmentEnd.y)
  )
}

function doSegmentsIntersect(params: {
  firstStart: Point2d
  firstEnd: Point2d
  secondStart: Point2d
  secondEnd: Point2d
}): boolean {
  const firstStartSide = getOrientation({
    first: params.firstStart,
    second: params.firstEnd,
    third: params.secondStart,
  })
  const firstEndSide = getOrientation({
    first: params.firstStart,
    second: params.firstEnd,
    third: params.secondEnd,
  })
  const secondStartSide = getOrientation({
    first: params.secondStart,
    second: params.secondEnd,
    third: params.firstStart,
  })
  const secondEndSide = getOrientation({
    first: params.secondStart,
    second: params.secondEnd,
    third: params.firstEnd,
  })
  ...
  return (
    isPointOnSegment({
      point: params.secondStart,
      segmentStart: params.firstStart,
      segmentEnd: params.firstEnd,
    }) ||
    isPointOnSegment({
      point: params.secondEnd,
      segmentStart: params.firstStart,
      segmentEnd: params.firstEnd,
    }) ||
    isPointOnSegment({
      point: params.firstStart,
      segmentStart: params.secondStart,
      segmentEnd: params.secondEnd,
    }) ||
    isPointOnSegment({
      point: params.firstEnd,
      segmentStart: params.secondStart,
      segmentEnd: params.secondEnd,
    })
  )
...
function getSquaredDistanceBetweenSegments(params: {
  firstStart: Point2d
  firstEnd: Point2d
  secondStart: Point2d
  secondEnd: Point2d
}): number {
  if (doSegmentsIntersect(params)) return 0
  return Math.min(
...
function doesSegmentIntersectRect(params: {
  segmentStart: Point2d
  segmentEnd: Point2d
  rect: RectBounds
}): boolean {
  if (
    getSquaredDistanceFromPointToRect({
      point: params.segmentStart,
      rect: params.rect,
    }) === 0 ||
    getSquaredDistanceFromPointToRect({
      point: params.segmentEnd,
      rect: params.rect,
    }) === 0
  ) {
    return true
  ...
  const corners: Point2d[] = [
    { x: params.rect.minX, y: params.rect.minY },
    { x: params.rect.maxX, y: params.rect.minY },
    { x: params.rect.maxX, y: params.rect.maxY },
    { x: params.rect.minX, y: params.rect.maxY },
  ]
  for (let cornerIndex = 0; cornerIndex < corners.length; cornerIndex++) {
    const nextCornerIndex = (cornerIndex + 1) % corners.length
    if (
      doSegmentsIntersect({
        firstStart: params.segmentStart,
        firstEnd: params.segmentEnd,
        secondStart: corners[cornerIndex]!,
        secondEnd: corners[nextCornerIndex]!,
      })
    ) {
      return true
    }
...
function doesCapsuleIntersectRect(params: {
  segmentStart: Point2d
  segmentEnd: Point2d
  radius: number
  rect: RectBounds
}): boolean {
  if (doesSegmentIntersectRect(params)) return true
  const radiusSquared = params.radius ** 2
  if (
    getSquaredDistanceFromPointToRect({
      point: params.segmentStart,
      rect: params.rect,
...
export class HighDensitySolverB01 extends BaseSolver {
  ...
  initialPenaltyFn?: HighDensitySolverB01Props["initialPenaltyFn"]
  ...
  private obstacleTraceRootIdsFlat!: Array<ObstacleRootId[] | undefined>
  private obstacleViaRootIdsByCell!: Array<ObstacleRootId[] | undefined>
  ...
  constructor(props: HighDensitySolverB01Props) {
    super()
  ...
  override getConstructorParams(): [HighDensitySolverB01Props] {
    return [
      {
  ...
  override _setup(): void {
    ...
    if (width > 15 || height > 15) {
      this.error = `HighDensitySolverB01 only supports routing windows up to 15x15mm; received ${width}x${height}mm`
      this.failed = true
      return
    }
    ...
    this.obstacleTraceRootIdsFlat = Array.from(
      { length: totalCells },
      () => undefined,
    )
    this.obstacleViaRootIdsByCell = Array.from(
  ...
  private rasterizeObstacles(): string | null {
    for (
      let obstacleIndex = 0;
      obstacleIndex < this.obstacles.length;
      obstacleIndex++
    ) {
      ...
      if (
        ...
        obstacle.route.length < 2
      ) {
        return `Obstacle ${obstacleIndex} must have positive trace/via sizes and at least two route points`
      }
      const rootConnectionName = toRootNetName(
        obstacle.connectionName,
        obstacle.rootConnectionName,
      )
      const obstacleRootId = this.internObstacleRootName(rootConnectionName)

      for (
        let routePointIndex = 1;
        routePointIndex < obstacle.route.length;
        routePointIndex++
      ) {
        const segmentStart = obstacle.route[routePointIndex - 1]!
        const segmentEnd = obstacle.route[routePointIndex]!
        if (
        ...
        if (segmentStart.z === segmentEnd.z) {
          const layer = this.zToLayer.get(segmentStart.z)
          if (layer === undefined) {
            return `Obstacle ${obstacleIndex} uses unavailable layer z=${segmentStart.z}`
          }
          this.rasterizeObstacleTraceSegment({
            segmentStart,
            segmentEnd,
            layer,
            obstacleRootId,
    ...
    this.obstacleTraceBlockedCellCount = this.obstacleTraceRootIdsFlat.reduce(
  ...
  private internObstacleRootName(
    rootConnectionName: RootConnectionName,
  ): ObstacleRootId {
    const existingObstacleRootId =
      this.obstacleRootNameToId.get(rootConnectionName)
    if (existingObstacleRootId !== undefined) {
      return existingObstacleRootId
    }
    const obstacleRootId = this.obstacleRootNames.length
    this.obstacleRootNames.push(rootConnectionName)
    this.obstacleRootNameToId.set(rootConnectionName, obstacleRootId)
    return obstacleRootId
  }

  private rasterizeObstacleTraceSegment(params: {
    segmentStart: Point2d
    segmentEnd: Point2d
    layer: number
    obstacleRootId: ObstacleRootId
    obstacleTraceThickness: number
  }): void {
    this.obstacleTracePrimitives.push({
      rootId: params.obstacleRootId,
      layer: params.layer,
      segmentStart: params.segmentStart,
      segmentEnd: params.segmentEnd,
      traceRadius: params.obstacleTraceThickness / 2,
    })
    const gridSegmentStart = this.transformBoundsPointToGrid(
      params.segmentStart,
    )
    const gridSegmentEnd = this.transformBoundsPointToGrid(params.segmentEnd)
    const traceClearanceRadius = this.getConservativeGridRadius(
      params.obstacleTraceThickness / 2 + this.traceMargin,
    )
    const viaClearanceRadius = this.getConservativeGridRadius(
      params.obstacleTraceThickness / 2 +
        this.viaDiameter / 2 +
        this.traceMargin,
    )

    this.forEachCellNearSegment(
      {
        segmentStart: gridSegmentStart,
        segmentEnd: gridSegmentEnd,
        radius: traceClearanceRadius,
      },
      (cellId) => {
        this.addObstacleRootId({
          rootIdsByCell: this.obstacleTraceRootIdsFlat,
          cellIndex: params.layer * this.planeSize + cellId,
  ...
  private rasterizeObstacleVia(params: {
    center: Point2d
    obstacleRootId: ObstacleRootId
    ...
    this.forEachCellNearCircle(
      gridCenter.x,
      gridCenter.y,
      Math.max(traceClearanceRadius, viaClearanceRadius),
      (cellId) => {
        const cellRect = this.getCellRect(cellId)
        if (
          circleIntersectsRect(
            gridCenter.x,
            gridCenter.y,
            traceClearanceRadius,
            cellRect.minX,
            cellRect.minY,
            cellRect.maxX,
            cellRect.maxY,
  ...
  private forEachCellNearSegment(
    ...
    const midpoint = {
      x: (params.segmentStart.x + params.segmentEnd.x) / 2,
      y: (params.segmentStart.y + params.segmentEnd.y) / 2,
    }
    const segmentRadius =
      Math.hypot(
        params.segmentEnd.x - params.segmentStart.x,
        params.segmentEnd.y - params.segmentStart.y,
      ) /
        2 +
      params.radius
    this.forEachCellNearCircle(
  ...
  private getCellRect(cellId: number): RectBounds {
    return {
      minX: this.cellMinX[cellId]!,
      minY: this.cellMinY[cellId]!,
      maxX: this.cellMaxX[cellId]!,
      maxY: this.cellMaxY[cellId]!,
    }
  }

  private transformBoundsPointToGrid(point: Point2d): Point2d {
    const transform = this.gridToBoundsTransform
    return {
      x: (point.x - transform.c) / transform.a,
      y: (point.y - transform.f) / transform.e,
    }
  }

  private getConservativeGridRadius(boundsRadius: number): number {
    return (
      boundsRadius /
      Math.min(
        Math.abs(this.gridToBoundsTransform.a),
        Math.abs(this.gridToBoundsTransform.e),
      )
    )
  }

  private addObstacleRootId(params: {
    rootIdsByCell: Array<ObstacleRootId[] | undefined>
    cellIndex: number
    obstacleRootId: ObstacleRootId
  }): void {
    let obstacleRootIds = params.rootIdsByCell[params.cellIndex]
    if (!obstacleRootIds) {
      obstacleRootIds = []
      params.rootIdsByCell[params.cellIndex] = obstacleRootIds
    }
    pushUnique(obstacleRootIds, params.obstacleRootId)
  }

  private isBlockedByObstacleRoots(
    ...
    for (const obstacleRootId of obstacleRootIds) {
      if (this.obstacleRootNames[obstacleRootId] !== activeRootConnectionName) {
        return true
      }
    }
    return false
  ...
  private isLateralMoveBlockedByObstacleGeometry(params: {
    ...
    const activeRootConnectionName = this.connIdToRootNet[params.activeConn]
    const activeSegment = this.activeConnSeg
    const segmentStart =
      activeSegment &&
      params.layer === activeSegment.startZ &&
      params.fromCellId === activeSegment.startCellId
        ? activeSegment.startPoint
        : applyAffineTransformToPoint(this.gridToBoundsTransform, {
            x: this.cellCenterX[params.fromCellId]!,
            y: this.cellCenterY[params.fromCellId]!,
          })
    const segmentEnd =
      activeSegment &&
      params.layer === activeSegment.endZ &&
      params.toCellId === activeSegment.endCellId
        ? activeSegment.endPoint
        : applyAffineTransformToPoint(this.gridToBoundsTransform, {
            x: this.cellCenterX[params.toCellId]!,
            y: this.cellCenterY[params.toCellId]!,
          })

    for (const obstacleTrace of this.obstacleTracePrimitives) {
      if (
        obstacleTrace.layer !== params.layer ||
        this.obstacleRootNames[obstacleTrace.rootId] ===
      ...
      if (
        getSquaredDistanceBetweenSegments({
          ...
          firstEnd: segmentEnd,
          secondStart: obstacleTrace.segmentStart,
          secondEnd: obstacleTrace.segmentEnd,
        }) <
  ...
  private isViaMoveBlockedByObstacleGeometry(params: {
    activeConn: ConnId
    cellId: number
  }): boolean {
    const activeRootConnectionName = this.connIdToRootNet[params.activeConn]
    const center = applyAffineTransformToPoint(this.gridToBoundsTransform, {
      x: this.cellCenterX[params.cellId]!,
      y: this.cellCenterY[params.cellId]!,
    })
    ...
    for (const obstacleTrace of this.obstacleTracePrimitives) {
      ...
      if (
        getSquaredDistanceFromPointToSegment({
          point: center,
          segmentStart: obstacleTrace.segmentStart,
          segmentEnd: obstacleTrace.segmentEnd,
        }) <
        requiredDistance * requiredDistance
      ) {
        return true
  ...
  private computeMoveCostAndRips(
    ...
    const obstacleRootIds = isVia
      ? this.obstacleViaRootIdsByCell[toCellId]
      : [
          ...(this.obstacleTraceRootIdsFlat[
            toZ * this.planeSize + fromCellId
          ] ?? []),
          ...(this.obstacleTraceRootIdsFlat[toFlatIdx] ?? []),
        ]
    const isBlockedByObstacle =
      this.isBlockedByObstacleRoots(obstacleRootIds, activeConn) &&
      (isVia
        ? this.isViaMoveBlockedByObstacleGeometry({
            activeConn,
            cellId: toCellId,
          })
        : this.isLateralMoveBlockedByObstacleGeometry({
            activeConn,
            layer: toZ,
            fromCellId,
            toCellId,
  ...
  override visualize() {
    ...
    return {
      ...
      title: `HighDensityB01 [${this.getSolvedRouteCount()} solved, ${this.unsolvedSegs?.length ?? 0} remaining]`,
    }
  }
